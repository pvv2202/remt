        ...
        for r in c.res:
            if match(m.seq, r.seq):
                ...
                if args.mt_in_range is not None:
                    for mt_in_range in c.mts:
                        if args.mt_in_range is not None and distance(min(r.start, r.end), max(r.start, r.end), min(mt_in_range.start, mt_in_range.end), max(mt_in_range.start, mt_in_range.end)) < args.mt_in_range:
                            mt_count += 1
                temp = {
                    "Contig": c.id,
                    "REType": r.enzyme,
