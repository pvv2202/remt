...
#TODO: FACTOR IN THE FACT THAT SOME BINDING REGIONS ARE ONLY SENSITIVE TO THE EDGE
...
def calculate_similarity(string1, string2):
    set1 = set(string1.upper())
    set2 = set(string2.upper())
...
for item in data:
    ...
    if all(keyword not in desc for keyword in ["control protein", "homing endonuclease", "enzyme/methyltransferase", "subunit", "helicase", "nicking endonuclease"]) and "RecSeq:" in desc:        #Add contigs
    ...
    for m in c.mts:
        for r in c.res:
            if match(m.seq, r.seq):
                ...
                weighted_avg = round(calculate_weighted_average([m.score, r.score]) * calculate_similarity(m.seq, r.seq))
                temp = {
                    "Contig": c.id,
                    "REType": r.enzyme,
                    "MT": m.domain,
...
for c in contigs:
    ...
    if len(c.res) > 0 and len(c.mts) > 0:
        num_both += 1
