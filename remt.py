...
class enzyme:
    def __init__(self, contig, seq, start, end, score, domain, enzyme):
        self.contig = contig
        self.seq = seq
...
class methyl(enzyme):
    def __init__(self, contig, seq, start, end, score, domain, enzyme):
        super().__init__(contig, seq, start, end, score, domain, enzyme)
...
for item in data:
    ...
    if "RecSeq:" in desc and "control" not in desc and "homing" not in desc and "enzyme/methyltransferase" not in desc and "subunit" not in desc and "helicase" not in desc and "nicking" not in desc:
        i = desc.find("RecSeq:")
        ...
        for m in prefixes:
            if item.get("domain").startswith(m):
                methyls.append(methyl(item.get("contig"), seq, item.get("start"), item.get("end"), item.get("score"), item.get("domain"), et))
                isRE = False
        if isRE:
            res.append(re(item.get("contig"), seq, item.get("start"), item.get("end"), item.get("score"), item.get("domain"), et))
    ...
    for r in res:
        if match(m.seq, r.seq) and r.contig == m.contig:
            ...
            if args.mt_in_range is not None:
                for mt_in_range in methyls:
                    if args.mt_in_range is not None and distance(min(r.start, r.end), max(r.start, r.end), min(mt_in_range.start, mt_in_range.end), max(mt_in_range.start, mt_in_range.end)) < args.mt_in_range and r.contig == mt_in_range.contig:
                        mt_count += 1
            temp = {
                "Contig": m.contig,
                "REType": r.enzyme,
